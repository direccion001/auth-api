...
router.use("/prospectos", requireAuth, (req, res, next) => {
  ...
  return res.status(403).json({ ok: false, code: "PROSPECTOS_SOLO_ADMIN_DIRECTIVO", message: "Solo Admin y Directivo pueden modificar Prospectos." });
});
...
router.patch("/prospectos/:id_appsheet", requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    if (statusActual === "2 Inscrito" && statusSolicitado !== "2 Inscrito") {
      return res.status(409).json({ ok: false, code: "STATUS_CONTACTO_INSCRITO_BLOQUEADO", message: "Este prospecto ya fue inscrito. Su status de contacto no puede cambiarse desde Company Viewer." });
    }
    return next();
  } catch (error) {
    console.error("[CRUD PROSPECTOS] Error validando status de inscrito", error);
    return res.status(500).json({ ok: false, code: "ERROR_VALIDANDO_STATUS_INSCRITO", message: "No pudimos validar el status del prospecto." });
  }
});
